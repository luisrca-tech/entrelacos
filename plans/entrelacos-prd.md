...
**Status:** Accepted product scope consolidated from the 2026-09-09 interview, with PIN-only guest confirmation. Architecture is documented separately; implementation remains unimplemented.
...
Guests visit the public wedding URL. A representative of a self-service family or group must identify the invitation with full name and registered Brazilian phone number, then enter the persistent six-digit group PIN that the bride or planner shared with the invitation link. The representative receives a family-scoped session and can answer RSVP for each member and publish one group message when those features are enabled. The API remains responsible for authorization, tenant isolation, business rules, rate limits, and session invalidation. There is no automated SMS or messaging-provider confirmation path.
...
- Guests are public visitors. A family or group representative may access protected guest actions only after exact lookup by full name plus registered phone and six-digit group-PIN verification; other group members do not need accounts.
...
The `OWNER` is the global operator account. It can read, create, and manage all weddings, users, site lifecycle status, domains and origins, operational data, review approval, terms, and support corrections. Only the owner can read across-wedding or global operational data. There is no public owner signup. Owner bootstrap and recovery use a restricted internal command or equivalent controlled procedure.
...
The representative is the one contact for a group or individual invitation. The representative uses full name and registered phone to create a short-lived verification challenge, enters the six-digit group PIN shared by the bride or planner, answers members' RSVP, and publishes or edits the group's one message while the mural is enabled. The representative cannot add or remove members, change group composition, edit another group, or access any other wedding.
...
Retired story IDs remain as explicit tombstones where historical block references could otherwise become ambiguous. They are not active requirements, implementation work, or launch gates.
...
49. **US-049 — As a `SITE_ADMIN`, I want to mark a foreign-number group, so that an administrative RSVP can be recorded when guest self-service is unavailable.**
50. **US-050 — As a foreign-number invitee, I want a persistent explanation that guest self-service is unavailable for my case, so that I know why the host must record the RSVP.**
...
54. **US-054 — As a representative, I want to enter the six-digit group PIN shared with my invitation, so that I can verify access through the product's manual confirmation flow.**
...
56. **US-056 — RETIRED — Former optional SMS resend story.** The accepted product has no automated resend or messaging-provider flow; PIN confirmation is manual.
57. **US-057 — As an operator, I want failed-PIN limits by wedding/group and phone, with additional IP throttling, so that abuse is constrained without changing RSVP data.**
58. **US-058 — As a guest, I want five incorrect PIN attempts to interrupt the challenge and apply a 15-minute cooldown, so that an attacker cannot keep guessing indefinitely.**
59. **US-059 — RETIRED — Former resend-counter story.** The accepted manual PIN challenge has no resend action.
...
65. **US-065 — RETIRED — Former demo verification-grant story.** The demo uses the same exact lookup and manual PIN confirmation as every other site; it has no bypass or provider simulation.
...
82. **US-082 — RETIRED — Former monthly messaging-quota story.** RSVP, sessions, messages, and reports have no SMS quota dependency.
...
98. **US-098 — RETIRED — Former monthly SMS send-ceiling story.** The accepted product has no automated messaging send or quota.
99. **US-099 — RETIRED — Former SMS usage-alert story.** The accepted product has no automated messaging usage to alert on.
100. **US-100 — RETIRED — Former blocked-SMS guidance story.** Guest confirmation uses the manual PIN and has no provider-send state.
101. **US-101 — RETIRED — Former demo SMS-simulation story.** The demo follows the same manual PIN flow and has no send simulation.
102. **US-102 — RETIRED — Former mocked-SMS test story.** Tests cover deterministic manual PIN behavior and do not model SMS delivery.
103. **US-103 — RETIRED — Former Twilio Verify story.** No Twilio or other messaging-provider confirmation path is part of the accepted product.
...
123. **US-123 — RETIRED — Former provider-result distinction story.** There is no simulated or live messaging-provider result in the accepted product.
124. **US-124 — As a service owner, I want unresolved legal, recovery, media, and infrastructure decisions to appear as technical launch gates while commercial policy is tracked separately, so that implementation cannot silently turn open questions into promises.**
...
Foreign-number groups may be recorded without a usable Brazilian phone and are administrative RSVP only. They do not use guest lookup, guest PIN confirmation, guest messages, or a substitute guest account. The interface keeps the explanation visible and provides a Sonner toast when the foreign-number path affects an action. Every self-service invitation requires full name plus its registered phone for lookup.
...
Manual verification applies exact lookup, failed-PIN and IP attempt throttles, and five incorrect PIN attempts before a 15-minute cooldown. Limits do not delete RSVP or interrupt existing valid sessions. Guest sessions expire absolutely after seven days. Representative/phone changes and PIN rotation revoke family sessions and pending challenges; spelling-only corrections do not.
...
- The first production infrastructure is manual. Cloudflare account/app setup, Railway deployment configuration, Neon environments, domains, DNS, and approved media-provider decisions are recorded and verified separately. No messaging provider is configured or required.
...
- Configuration examples may contain public values, but secrets must remain in the intended secret stores. Frontend bundles must never receive Neon, Railway, Better Auth, or other server credentials.
...
- TDD applies to logic-bearing behavior. Tests use deterministic fixtures. A mocked media provider is test infrastructure, not evidence of a live provider guarantee.
...
- a representative can locate an invitation with full name plus registered phone and complete the manual six-digit PIN verification;
...
- tests show cross-tenant isolation, PIN abuse limits, burst/load behavior, and clean-browser session behavior;
- recovery, legal/retention, media rights, and production infrastructure gates are either evidenced or explicitly marked blocked before launch.
...
- Automated SMS, WhatsApp API integration or authentication, automated messaging fallback, and any messaging-provider integration in the MVP. Administrators may manually paste the copied group PIN into their own communication channel.
...
- Guaranteed capacity, guaranteed RPO/RTO, legal compliance, media licensing, or third-party service behavior without evidence and a completed gate.
...
2. Neon tier, retention settings, cost, backup strategy, and real restoration proof for RPO/RTO targets.
3. Privacy notice, retention schedule, data rights handling, deletion exceptions, client media permissions, and legal review.
4. Authenticated cross-origin design between public wedding sites, panel, and API, including same-origin/BFF/cookie or time-bound handoff choice and clean-browser proof.
5. Cloudflare, Railway, Neon, and CI accounts, credentials, project access, and environment permissions.
6. Media provider, image/video rights, costs, API keys, performance budget, fictional-couple approval, and image-to-video poster workflow.
7. Final visual direction, brand/logo/favicons, and acceptance of the provisional palette and motion vocabulary.
8. Operational scale beyond the target and the response to Cloudflare's Worker-per-wedding account limits.
9. Production domain and DNS responsibilities, including custom-domain ownership and renewal policy.

## Further Notes

This PRD records the accepted product interview and the PIN-only guest-confirmation decision. The architecture specification is a separate artifact. The implementation plan paired with it is deliberately marked **DRAFT** because the eight proposed blocks require a user granularity review before work is scheduled.
