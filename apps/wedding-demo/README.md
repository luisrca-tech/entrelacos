...
The guest-access island locates a group by exact normalized full name plus its registered Brazilian phone, verifies the six-digit PIN shared with the invitation, and stores the resulting family bearer only in site-namespaced `sessionStorage`. The demo uses the same provider-free PIN flow as every wedding.
