...
export function InvitationFormDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className={`${adminStyles.dialog} max-w-[min(720px,calc(100vw-32px))]`}
      >
        ...
        >
          ...
          <fieldset className="grid gap-3 border-t border-admin-line pt-5">
            ...
            {fields.map((field, index) => (
              ...
              >
                {field.id && (
                  <input type="hidden" {...register(`guests.${index}.id`)} />
                )}
            ...
            {errors.guests?.root && (
              <span role="alert" className="text-xs text-admin-status-declined">
                Adicione pelo menos um convidado.
              </span>
            )}
          </fieldset>
          <div className="sticky bottom-0 flex justify-end gap-2 border-t border-admin-line bg-admin-surface py-3">
            <Button
              type="button"
              variant="outline"
