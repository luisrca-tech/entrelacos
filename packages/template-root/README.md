...
`StoryContent` keeps one short host-owned story paired with one background media asset. The section renders a full-bleed image or video with a readable text overlay; additional couple photos belong in `GalleryContent`. The media uses the same supplied dimensions and poster fallback contract as hero media.
