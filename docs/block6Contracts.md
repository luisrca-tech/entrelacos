...
- `ScheduleContent` requires at least two entries and may include host-owned practical guidance. A guidance item may include an optional detail opened from a host-owned label: title, body, close label, and one image. Times are display strings supplied by the host; the template does not interpret timezone or event semantics.
